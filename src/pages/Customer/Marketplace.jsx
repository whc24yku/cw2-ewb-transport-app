...
const CustomerMarketplace = () => {
  ...
  // Sample items to showcase; replace with dynamic data as needed.
  const items = [
    {
      id: 1,
  ...
  const categories = ['All', 'Food', 'Artisanal Goods', 'Tours', 'Fishes', 'Meats', 'Groceries'];
    ...
    <div className="fixed inset-0 bg-gradient-to-r from-[#027f86] to-purple-600 text-white flex flex-col m-0 p-0 overflow-hidden">
      ...
      <header className="flex justify-between items-center mb-6 px-4 py-2 w-full">
        ...
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-semibold text-white ml-3">Marketplace</h1>
        </div>
      ...
      <div className="flex items-center justify-between mb-4 px-4 py-2">
        ...
        <div className="flex space-x-3">
          <button onClick={() => setViewType('grid')}>
            <FontAwesomeIcon icon={faTh} className={`text-2xl ${viewType === 'grid' ? 'text-yellow-300' : 'text-gray-300'}`} />
          </button>
          <button onClick={() => setViewType('list')}>
            <FontAwesomeIcon icon={faList} className={`text-2xl ${viewType === 'list' ? 'text-yellow-300' : 'text-gray-300'}`} />
          </button>
        </div>
      </div>

      {/* Items Section */}
      <div className={`${viewType === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 py-2' : 'flex flex-col space-y-4 px-4 py-2'}`}>
        {items
          .filter(
            (item) =>
          ...
          .map((item) => (
            <div key={item.id} className="bg-white text-[#027f86] rounded-lg shadow-lg p-4 flex items-start space-x-4">
              ...
              <div className="flex flex-col justify-between">
                ...
                <div className="mt-2">
                  ...
                  <button className="mt-2 bg-yellow-300 text-[#027f86] px-4 py-2 rounded-md font-semibold hover:bg-yellow-400 transition">
                    Buy Now
                  </button>
                </div>
              </div>
