// src/pages/Admin/AdminBookTransport.jsx

import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faTruck, faFish, faAppleAlt, faHorse, faCar, faHeartbeat, faMapMarkerAlt, faUser } from '@fortawesome/free-solid-svg-icons';

const AdminBookTransport = () => {
  const navigate = useNavigate();
  const [onBehalfOf, setOnBehalfOf] = useState(''); // New field for booking on behalf of a user
  ...
  const handleBooking = () => {
    // Placeholder function to handle booking
    console.log({ onBehalfOf, transportType, vehicleType, isPriority });
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-r from-[#027f86] to-purple-600 text-white flex flex-col items-center overflow-hidden p-0 m-0">
      ...
      <div className="flex items-center my-4">
        ...
        <h1 className="text-lg font-semibold text-white ml-2">GoLobitos Admin</h1>
      </div>
      ...
      <div className="w-full max-w-lg bg-white text-[#027f86] rounded-lg shadow-lg p-4 mx-2 mb-4 overflow-y-auto box-border max-h-[75vh]">
        <h2 className="text-2xl font-semibold text-center mb-3">Book a Ride (Admin)</h2>

        {/* On Behalf Of Field */}
        <div className="mb-3">
          <label className="block text-md mb-1">On Behalf Of</label>
